...
test.describe('TC-AUTH: Módulo de Autenticación', () => {
  ...
  const createAccountBtn = page.locator('#show-register');

  await createAccountBtn.click();

  await page.waitForTimeout(3000);

  const registerContainer = page.locator('#register-screen');
  ...
  await page.locator('#r-password').fill('Test1234!');
...
test('TC-AUTH-006: Registrar usuario con contraseña corta', async ({ page }) => {
  ...
  await page.locator('#r-password').fill('123');
